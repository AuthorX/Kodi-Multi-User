...
-- CREATE DATABASE --
CREATE DATABASE IF NOT EXISTS `Kodi_U01_Video_121` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
-- SET USER ACCESS --
GRANT ALL PRIVILEGES ON `Kodi_U01_Video_121`.* TO 'KODI_21'@'%' WITH GRANT OPTION;
-- SET ACTIVE DATABASE --
USE `Kodi_U01_Video_121`;
  ...
  -- TABLE STRUCTURE FOR TABLES --
    ...
    INSERT INTO `version` (`idVersion`, `iCompressCount`) VALUES (121, 0);
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_person` AFTER DELETE ON `actor`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_episode` AFTER DELETE ON `episode`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_file` AFTER DELETE ON `files`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_movie` AFTER DELETE ON `movie`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_musicvideo` AFTER DELETE ON `musicvideo`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_season` AFTER DELETE ON `seasons`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_set` AFTER DELETE ON `sets`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_tag` AFTER DELETE ON `tag_link`
  FOR EACH ROW
  BEGIN
  ...
  DELIMITER $$
  CREATE DEFINER=`KODI_21`@`%` TRIGGER `delete_tvshow` AFTER DELETE ON `tvshow`
  FOR EACH ROW
  BEGIN
